...
function PostThumb({ post, index }) {
  ...
  const showPlaceholder = !post.media_url || imgError
    ...
    >
      {!showPlaceholder
        ? <img
          ...
          />
        : <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5 bg-gradient-to-br from-charcoal-800 to-charcoal-900">
            {post.media_type === 'VIDEO'
              ? <Play size={20} className="text-gold-400/40 fill-gold-400/40" />
...
export default function InstagramFeed({ limit = 18, showHeader = true }) {
